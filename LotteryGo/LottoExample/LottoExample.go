...
//LottoExample : Created for testing goroutines & multiple go files.
func LottoExample(messages chan<- string, wg *sync.WaitGroup) {
	defer wg.Done()
	start := time.Now()
	defer func() {
		fmt.Println("LottoExample time elapsed:", time.Since(start))
	}()
	fileLoc := "LottoExample.json"
	writeFileLoc := "LottoExampleSingles.txt"
